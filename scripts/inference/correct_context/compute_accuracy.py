...
def build_chat_text(tokenizer, question, answer, candidate):
    messages = [
...
def judge_batch(tokenizer, model, examples):
    texts = [
        build_chat_text(tokenizer, question, answer, candidate)
        for question, answer, candidate in examples
    ]
    ...
    return [parse_bool(output_text) for output_text in output_texts]
...
def process_file(result_file, dataset, tokenizer, model, batch_size):
    ...
    for qid in tqdm(json_file, desc=os.path.basename(result_file)):
        ...
        batch.append((result, (question, ground_truth, predicted_answer)))
